local utils = require("utils")

vim.keymap.set("n", "<leader><leader>", ":")
vim.keymap.set("n", "<C-p>", ":")

vim.keymap.set("n", "j", "v:count == 0 ? 'gj' : 'j'", { expr = true, silent = true })
vim.keymap.set("n", "k", "v:count == 0 ? 'gk' : 'k'", { expr = true, silent = true })

vim.keymap.set("n", "<C-a>", "ggVG<cr>", { desc = "Select all" })

vim.keymap.set({ "i", "v", "n" }, "<C-s>", "<cmd>w<cr><esc>", { desc = "Save file" })

vim.keymap.set("n", "<esc>", "<cmd>noh<cr>")

vim.keymap.set("v", "<", "<gv")
vim.keymap.set("v", ">", ">gv")

vim.keymap.set("v", "p", '"_dP')

vim.keymap.set("n", "<C-M-j>", ":m .+1<cr>==", { desc = "Move down" })
vim.keymap.set("v", "<C-M-j>", ":m '>+1<cr>gv=gv", { desc = "Move down" })
vim.keymap.set("i", "<C-M-j>", "<Esc>:m .+1<cr>==gi", { desc = "Move down" })
vim.keymap.set("n", "<C-M-k>", ":m .-2<cr>==", { desc = "Move up" })
vim.keymap.set("v", "<C-M-k>", ":m '<-2<cr>gv=gv", { desc = "Move up" })
vim.keymap.set("i", "<C-M-k>", "<Esc>:m .-2<cr>==gi", { desc = "Move up" })

vim.keymap.set({ "i", "v", "n" }, "<C-w>", "<cmd>bd<cr><esc>", { desc = "Close buffer" })
vim.keymap.set({ "i", "v", "n" }, "<C-M-w>", "<cmd>bd!<cr><esc>", { desc = "Close buffer" })

vim.keymap.set({ "i", "v", "n" }, "<C-q>", "<cmd>q<cr>", { desc = "Exit Vim" })
vim.keymap.set({ "i", "v", "n" }, "<C-M-q>", "<cmd>qa!<cr>", { desc = "Exit Vim" })

vim.keymap.set("n", "<C-d>", "<C-d>zz")
vim.keymap.set("n", "<C-u>", "<C-u>zz")

vim.keymap.set("n", "<leader>f", "za")


vim.keymap.set("n", "<leader>tw", function()
  utils.toggle("wrap")
end, { desc = "Toggle Word Wrap" })
vim.keymap.set("n", "<leader>ts", function()
  utils.toggle("spell")
end, { desc = "Toggle Spelling" })
vim.keymap.set("n", "<leader>tl", function()
  utils.toggle("relativenumber")
end, { desc = "Toggle Line Numbers" })
vim.keymap.set("n", "<leader>td", utils.toggle_diagnostics, { desc = "Toggle Diagnostics" })
vim.keymap.set("n", "<leader>q", utils.toggle_quickfix, { desc = "Toggle Quickfix Window" })

vim.keymap.set("n", "<C-M-f>", function()
  vim.lsp.buf.format({ async = false })
  vim.api.nvim_command("write")
end, { desc = "Lsp formatting" })

vim.api.nvim_set_keymap('n', '<Tab>', [[:lua SwitchToPreviousBuffer()<CR>]], { noremap = true, silent = true })

vim.api.nvim_set_keymap('n', '<CR>', [[:lua SwitchToNextBuffer()<CR>]], { noremap = true, silent = true })

function SwitchToPreviousBuffer()
  local current_buffer = vim.fn.bufnr('%') -- Get the current buffer number

  repeat
    current_buffer = current_buffer - 1
    if current_buffer < 1 then
      current_buffer = vim.fn.bufnr('$') -- Wrap around to the last buffer if reached the first buffer
    end

    local bufname = vim.fn.bufname(current_buffer)
    if vim.fn.buflisted(current_buffer) == 1 and bufname ~= '' and bufname ~= '[No Name]' and not vim.startswith(bufname, '[') then
      vim.cmd('buffer ' .. current_buffer)
      break
    end
  until current_buffer == vim.fn.bufnr('%')
end

function SwitchToNextBuffer()
  local current_buffer = vim.fn.bufnr('%') -- Get the current buffer number

  repeat
    current_buffer = current_buffer + 1
    if current_buffer > vim.fn.bufnr('$') then
      current_buffer = 1 -- Wrap around to the first buffer if reached the last buffer
    end

    local bufname = vim.fn.bufname(current_buffer)
    -- Check if the buffer is listed, not named "[No Name]", and its name does not start with '['
    if vim.fn.buflisted(current_buffer) == 1 and bufname ~= '' and bufname ~= '[No Name]' and not vim.startswith(bufname, '[') then
      vim.cmd('buffer ' .. current_buffer)
      break
    end
  until current_buffer == vim.fn.bufnr('%')
end
